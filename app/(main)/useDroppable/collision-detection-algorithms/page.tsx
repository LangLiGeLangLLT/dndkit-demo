...
const algorithms = [
  { label: 'Rect Intersection', value: 'rectIntersection' },
  { label: 'Closest Center', value: 'closestCenter' },
  { label: 'Closest Corners', value: 'closestCorners' },
  { label: 'Pointer Within', value: 'pointerWithin' },
]
...
function Story() {
  ...
  const [collisionDetectionAlgorithm, setCollisionDetectionAlgorithm] =
    useState<string>('rectIntersection')

  const item = <DraggableItem />

  const collisionDetection = useMemo(() => {
    if (collisionDetectionAlgorithm === 'rectIntersection') {
      return rectIntersection
    }
    if (collisionDetectionAlgorithm === 'closestCenter') {
      return closestCenter
    }
    if (collisionDetectionAlgorithm === 'closestCorners') {
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-col space-y-2">
        <h2>Collision detection algorithm</h2>
        <RadioGroup
          defaultValue={collisionDetectionAlgorithm}
          onValueChange={(value) => {
            setCollisionDetectionAlgorithm(value)
          }}
        >
